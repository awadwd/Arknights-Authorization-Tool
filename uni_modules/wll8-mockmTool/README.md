...
## 你为什么需要它？
- 就解决跨域方面，相比于 vue 或 react 的 devserver proxy 解决方案来说，它修改配置实时更新，不会造成前端界面的状态丢失，不需要 webpack。
- 在接口模拟方面，相比于 mockjs ，它不但可以自动生成数据，还能自动生成接口。可以在浏览器中看到和存储请求细节，并且可以避免 mockjs 拦截 xhr 造成的各种 bug。
- 在调试方面，集成了 swagger 相关的功能，方便查看接口文档，并且可以自动存储后端接口的修改记录，可以进行请求回放。
- 自带外网映射，不需要注册和登录就能获取域名、https，让你无需要担心提前对接例如微信这些平台。
