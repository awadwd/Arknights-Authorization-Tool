		...
		<view class="container">
				<view class="mode-section">
					...
					<view class="action-buttons">
						<button class="action-btn secondary" @click="toggleShowOverview"><text>{{ isGeneratingShareImage ? '生成中..' : '查看概览' }}</text></button>
				...
				<scroll-view class="boxes-scroll-view" scroll-y @scrolltolower="loadMore" @scroll="onPageScroll" :scroll-top="scrollTop">
					<view class="box-section" v-for="(box, index) in displayBoxes" :key="box && (box.Box_id || index)">
				...
				<view class="overview-page" v-if="showSharePreviewModal" id="overviewContent" :class="'overview-' + currentMode">
					<scroll-view class="overview-scroll" scroll-y>
						...
						<view class="overview-footer">
							<button class="generate-btn" @click="generateShareImageFromOverview">
				...
				<canvas canvas-id="shareCanvas" style="position: absolute; left: -9999px; top: -9999px; width: 750rpx; height: 2000rpx;" id="shareCanvas"></canvas>
	...
	methods: {
		...
		togglePair(character, boxId) {
			...
			if (!this.canHavePair(character)) {
				uni.showToast({ title: `${characterName}无法拥有一对`, icon: 'none' });
				return;
			}
		...
		getFilteredBoxesForOverview() {
			...
			return this.characterData.filter(box => {
				if (!box) return false;
				const boxType = box.Box_type || 'normal';
				if (filterType === 'normal') {
					return !boxType || boxType === 'normal';
		...
		generateShareImageFromOverview() {
			...
			// 使用 uni.createSelectorQuery 获得概览区域
			const query = uni.createSelectorQuery().in(this);

			query.select('#overviewContent')
				.fields({
					node: true,
					size: true,
					scrollOffset: true
				...
				.exec(async (res) => {
					...
					try {
						...
						// 绘制顶部装饰条
						canvas.fillStyle = this.currentMode === 'show' ? '#409EFF' : '#FF6B35';
						...
						canvas.fillText(this.currentMode === 'show' ? '🏆 收藏概览' : '📋 出物概览', canvasWidth / 2, 55);
						...
						canvas.fillRect(20, cardY, canvasWidth - 40, cardHeight);
						...
						const stats = [
							{ label: '已点亮', value: `${this.ownedCount}/${this.totalCharacters}` },
							{ label: '完成度', value: `${Math.round(this.ownedCount / this.totalCharacters * 100)}%` },
							{ label: '一对', value: this.pairCount },
							{ label: '单精一', value: this.singleElite1Count },
							{ label: '单精二', value: this.singleElite2Count }
						];
						...
						for (const stat of stats) {
							...
							canvas.fillText(stat.label, statX, cardY + 35);

							canvas.fillStyle = this.currentMode === 'show' ? '#409EFF' : '#FF6B35';
							canvas.font = 'bold 32px Arial';
							canvas.fillText(stat.value, statX, cardY + 75);
						...
						canvas.beginPath();
						canvas.moveTo(30, y);
						canvas.lineTo(canvasWidth - 30, y);
						canvas.stroke();
						...
						canvas.font = 'bold 32px Arial';
						canvas.textAlign = 'left';
						canvas.fillText('每盒进度', 30, y);

						y += 50;

						// 绘制每盒进度条
						const boxes = this.getFilteredBoxesForOverview();
						const progressBarHeight = 35;
						const progressBarGap = 50;
						...
						for (const box of boxes) {
							...
							canvas.textAlign = 'left';
							canvas.fillText(`盒号 ${box.Box_id}`, 30, y);

							canvas.fillStyle = '#666666';
							canvas.font = '20px Arial';
							canvas.textAlign = 'right';
							canvas.fillText(`${owned}/${total} (${Math.round(progress * 100)}%)`, canvasWidth - 30, y);

							y += 30;

							// 绘制进度条背景
							canvas.fillStyle = '#e0e0e0';
							canvas.fillRect(30, y, canvasWidth - 60, 20);

							// 绘制进度条填充
							const progressColor = this.currentMode === 'show' ? '#409EFF' : '#FF6B35';
							canvas.fillStyle = progressColor;
							canvas.fillRect(30, y, (canvasWidth - 60) * progress, 20);

							// 进度条边框
							canvas.strokeStyle = '#d0d0d0';
							canvas.lineWidth = 1;
							canvas.strokeRect(30, y, canvasWidth - 60, 20);
						...
						canvas.fillText(`生成时间${timeStr}`, canvasWidth / 2, y);
							...
							setTimeout(() => {
								uni.canvasToTempFilePath({
									...
									success: (res) => {
										...
										this.isGeneratingShareImage = false;
										// 显示分享图片弹窗
										this.showShareImagePreview = true;
									},
									fail: (err) => {
										console.error('杞崲失败:', err);
										uni.showToast({
											title: '生成失败，请重试',
											icon: 'none'
										});
		...
		// 切换概览显示
		toggleShowOverview() {
			this.showSharePreviewModal = !this.showSharePreviewModal;
		},
		...
		getShareCharacters(box) {
			if (!box) {
				console.warn('getShareCharacters: box为空');
				return [];
			}
			...
			// 根据当前模式筛选夎色插苟去婚噸
			for (const char of characters) {
				if (!char || !char.name || seenNames.has(char.name)) continue;

				if (this.currentMode === 'show') {
					// 点亮模式)氭樉绀烘墍链按凡拥有的角色
					if (this.isCharacterOwned(char.name)) {
						seenNames.add(char.name);
						uniqueCharacters.push(char);
					}
				} else {
					// 出物模式)氭樉绀烘墍链按嚭特╃殑角色
					if (this.isCharacterForTrade(char.name)) {
						seenNames.add(char.name);
						uniqueCharacters.push(char);
					}
				}
		...
		getTotalShareCharacters() {
			...
			for (const box of this.getShareBoxes()) {
				const characters = this.getShareCharacters(box);
				for (const char of characters) {
					if (char && char.name && !seenNames.has(char.name)) {
						seenNames.add(char.name);
						total++;
		...
		async generateShareImage() {
			...
			try {
				...
				// 根据角色数量动ㄦ总佽皟数村竷灞总
				let charactersPerRow = 4; // 默认每行4个
				let avatarSize = 40; // 默认头像大小
				let characterRowHeight = 90; // 默认每行高度害

				if (charactersCount > 40) {
					// 角色缩小，紧凑显示
					charactersPerRow = 6;
					avatarSize = 30;
					characterRowHeight = 70;
				} else if (charactersCount > 25) {
					charactersPerRow = 5;
					avatarSize = 35;
					characterRowHeight = 80;
				} else if (charactersCount > 16) {
					charactersPerRow = 4;
					avatarSize = 40;
					characterRowHeight = 90;
				} else if (charactersCount > 9) {
					charactersPerRow = 3;
					avatarSize = 45;
					characterRowHeight = 100;
				} else {
					charactersPerRow = 3;
					avatarSize = 50;
					characterRowHeight = 110;
				}

				const rows = Math.ceil(charactersCount / charactersPerRow);

				// 计算总高度
				const padding = 15;
				const titleHeight = 80;
				const characterRowsHeight = rows * characterRowHeight;
				const footerHeight = 100;

				let totalHeight = titleHeight + characterRowsHeight + footerHeight + 30; // 动态计算边距
				// 如果高度太小，保留最小高度
				...
				const subtitle = this.currentMode === 'show' ? '点亮模式 - 展示我的所有收藏' : '出物模式 - 这些角色可以交易';
				...
				// 已点亮数
				ctx.setFillStyle('#666666');
				ctx.fillText('已点亮数', padding, yPos);
				ctx.setFillStyle('#333333');
				...
				ctx.setFontSize(14);
				ctx.setFillStyle('#333333');
				ctx.fillText(`角色列表 (${charactersCount}个)`, canvasWidth / 2, yPos);
				...
				for (let i = 0; i < characters.length; i++) {
					...
					let fontSize = Math.min(12, avatarSize / 2.5);

					// 根据角色名字长度调整文字大小
					if (displayName.length > 4) {
						fontSize = Math.min(10, avatarSize / 3);
					} else if (displayName.length > 6) {
						fontSize = Math.min(8, avatarSize / 3.5);
					}
					...
					if (displayName.length > 6) {
						...
						displayText = displayName.substring(0, mid) + '\n' + displayName.substring(mid);

						// 绘制多行文本
						const lines = displayText.split('\n');
						let lineY = avatarY - (lines.length - 1) * fontSize / 2;

						for (let line of lines) {
							ctx.fillText(line, avatarX, lineY);
							lineY += fontSize * 1.2;
						}
					} else {
						// 短名字，直接显示
						ctx.fillText(displayText, avatarX, avatarY + fontSize / 3);
					}

					// 绘制角色名字：在下方显示完整名字
					const nameY = itemY + avatarSize + avatarMargin * 2 + 12;
					let nameFontSize = Math.min(10, characterItemWidth / 8);

					// 调整名字显示大小
					if (char.name.length > 4) {
						nameFontSize = Math.min(9, characterItemWidth / 10);
					} else if (char.name.length > 6) {
						nameFontSize = Math.min(8, characterItemWidth / 12);
					}

					ctx.setFontSize(nameFontSize);
					ctx.setFillStyle('#333333');

					// 显示完整角色名					let displayFullName = char.name;
					ctx.fillText(displayFullName, avatarX, nameY);

					// 绘制精英化等级标签					let tagsY = nameY + 10;
					let tagX = avatarX - 15;

					if (char.elite1) {
						ctx.setFillStyle('#E6A23C');
						ctx.fillRect(tagX, tagsY, 20, 10);
						ctx.setFontSize(8);
						ctx.setFillStyle('#ffffff');
						ctx.fillText('★', tagX + 10, tagsY + 8);
						tagX += 22;
					}

					if (char.elite2 && char.canHavePair) {
					...
					if (this.shareShowPrices) {
						...
						if (this.currentMode === 'trade' && char.tradePrices) {
							...
							if (char.tradePrices.elite1) {
								priceText = `${char.tradePrices.elite1}元`;
							} else if (char.tradePrices.elite2 && char.canHavePair) {
								priceText = `${char.tradePrices.elite2}元`;
							}

							if (priceText) {
								ctx.fillText(priceText, avatarX, priceY);
							}
							...
							if (char.marketPrices.elite1) {
								priceText = `${char.marketPrices.elite1}元`;
							} else if (char.marketPrices.elite2 && char.canHavePair) {
								priceText = `${char.marketPrices.elite2}元`;
							}
					...
					if (this.shareShowWanted && char.wanted) {
						...
						ctx.setFillStyle('#FF6B6B');
						ctx.setTextAlign('center');
						ctx.fillText('💜', avatarX + 12, itemY + avatarSize - 8);
					}
				}

				// 更新Y位置到角色列表底部
				currentY += characterRowHeight + 20;
				...
				try {
					...
					// 如果图片加载失败，用文字代替					ctx.setFontSize(11);
					ctx.setFillStyle('#999999');
					ctx.fillText('微信搜索"明日方舟通行证胶子查询工具"获得更多~', canvasWidth / 2, currentY);
					currentY += 15;
				}
				...
				ctx.setFontSize(10);
				ctx.setFillStyle('#999999');
				ctx.fillText('长按保存图片，分享给朋友', canvasWidth / 2, currentY);
		...
		prepareShareData() {
			...
			let characterNames = [];
			if (this.currentMode === 'show') {
				characterNames = Object.keys(this.ownedCharacters);
			} else {
				characterNames = Object.keys(this.tradeCharacters);
			}

			for (const characterName of characterNames) {
				if (seenNames.has(characterName)) continue;
				...
				const characterData = {
					name: character.name,
					elite1: this.isCharacterOwnedAtLevel(characterName, 'elite1'),
					elite2: this.isCharacterOwnedAtLevel(characterName, 'elite2'),
					wanted: this.isCharacterWanted(characterName),
					canHavePair: canHavePair
				};

				if (this.currentMode === 'trade' && this.isCharacterForTrade(characterName)) {
					characterData.tradePrices = {
						elite1: this.getTradePrice(characterName, 'elite1'),
						elite2: this.getTradePrice(characterName, 'elite2')
					};
				} else if (this.currentMode === 'show' && character.market_price) {
					characterData.marketPrices = {
						elite1: character.market_price.ELITE1,
						elite2: character.market_price.ELITE2
					};
				}
